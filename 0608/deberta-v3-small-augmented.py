...
class DebertaForRegression(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        attention_pooled_output = self.mean_pooling(outputs, attention_mask)
        return self.regression_head(attention_pooled_output)
...
augmented_data = pd.read_csv("additional_df_0607.csv")
augmented_data["score"] -= 1 
...
for fold, (train_idx, val_idx) in enumerate(skf.split(data["full_text"], data["score"], groups)):
    ...
    train_dataset = EssayDataset(augmented_full_text, augmented_scores)
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if kappa > best_kappa:
            best_kappa = kappa
            best_model_path = f"Augmented_MSE_DeBERTa-v3-small_{fold}_epoch{epoch}_kappa_{best_kappa}.bin"
