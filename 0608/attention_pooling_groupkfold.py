...
class DebertaForRegression(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        attention_pooled_output = self.attention_pooling(outputs, attention_mask)
...
for fold, (train_idx, val_idx) in enumerate(group_kfold.split(merged_df["full_text"], merged_df["score"], groups)):
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if kappa > best_kappa:
            best_kappa = kappa
            best_model_path = f"0608_{fold}_epoch{epoch}_kappa_{best_kappa}_AttentionPooling.bin"
