...
class DebertaForRegression(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        mean_pooled_output = torch.mean(outputs.last_hidden_state, dim=1)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(data["full_text"], data["score"])): 
    #if fold > 0: 
    #    break 
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if kappa > best_kappa:
            best_kappa = kappa
            best_model_path = f"240603_best_model_fold_{fold}.bin"
